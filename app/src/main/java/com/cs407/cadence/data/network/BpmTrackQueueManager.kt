...
/**
 * Manages background BPM checking and track queueing
 * Processes tracks one per second to avoid rate limiting
 * Maintains internal queue that can be cleared on new workout
 */
class BpmTrackQueueManager(
    ...
    private val internalQueue = mutableListOf<SpotifyTrack>() // Internal queue of BPM-matched tracks
    ...
    fun startProcessing(tracks: List<SpotifyTrack>) {
        ...
        Log.d(TAG, "Starting background BPM processing for ${tracks.size} tracks (target: $targetBpm±$bpmRange BPM)")
        ...
        try {
            ...
            if (bpm != null) {
                if (bpm in minBpm..maxBpm) {
                    Log.d(TAG, "✓ Match found: '${track.name}' (BPM: $bpm) - adding to internal queue")
